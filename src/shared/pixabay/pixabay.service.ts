...
export class PixabayService {
  ...
  async searchAndDownloadVideoStreams(
    ...
    try {
      ...
      const promises = hits
        .map((hit: any) => {
          // Pixabay provides various video qualities, pick the 'large' or 'medium' one
          // You might want to add more sophisticated logic here to pick the best quality
          const videoUrl = hit.videos.medium?.url || hit.videos.large?.url;
          if (videoUrl) {
            return this.fetchStream(videoUrl);
          }
          return null;
        })
        .filter(Boolean) as Promise<Readable>[];
    } catch (error) {
      this.logger.error(`Error searching for videos: ${error.message}`);
      return [];
    }
  }
   ...
   * @param videoSearchQuery The query string for video search.
   */
  async searchAndStoreVideoClips(
    ...
    this.logger.log(
      `[searchAndStoreVideoClips] Searching for video clips with query: ${videoSearchQuery}`,
    );
    console.time("search-and-store-video-clips");
    try {
      const videoStreams = await this.utilityService.retryOperation(
        () => this.searchAndDownloadVideoStreams(videoSearchQuery),
        "Pixabay video search and download",
      );

      if (videoStreams.length > 0) {
        const storedVideoClipIds: string[] = [];
        for (let i = 0; i < videoStreams.length; i++) {
          const videoStream = videoStreams[i];
          const videoClipId = await this.storageService.storeStream(
            videoStream,
            `video_${jobId}_${i}.mp4`,
          );
          storedVideoClipIds.push(videoClipId);
          this.logger.log(`Stored video clip with ID: ${videoClipId}`);
        }
        return storedVideoClipIds;
      } else {
        this.logger.warn("No suitable video file found for the given query.");
        return [];
      }
    } catch (error) {
      this.logger.error(
        `Error searching and storing video clips: ${error.message}`,
      );
      throw error;
    }
    console.timeEnd("search-and-store-video-clips");
  }
  //     if (!hits.length) {
