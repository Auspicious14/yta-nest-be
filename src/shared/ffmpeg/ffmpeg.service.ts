...
export class FfmpegService {
  ...
  async mergeAll(options: VideoMergeOptions): Promise<Readable> {
    ...
    try {
      ...
      const concatStream = await this.concatenateClips(
        clipStreams,
        tempConcatPath,
      );
      ...
      await fs
        .unlink(tempConcatPath)
        .catch((err) =>
          this.logger.warn(
            `Failed to delete temp file ${tempConcatPath}: ${err.message}`,
          ),
        );
  ...
  private async concatenateClips(
    ...
    return new Promise((resolve, reject) => {
      ...
      command
        .complexFilter(
          `[${filter}]concat=n=${clipStreams.length}:v=1:a=1[outv][outa]`,
        )
        .outputOptions([
          "-map",
          "[outv]",
      ...
      command
        .on("end", async () => {
          ...
          await fs.writeFile(
            outputPath,
            await this.streamToBuffer(outputStream),
          );
  ...
  private async mergeWithAudioAndMetadata(
    ...
    return new Promise((resolve, reject) => {
      ...
      if (musicStream) {
        filters.push(
          "[1:a]volume=1.0[mainAudio]",
          "[2:a]volume=0.2[bgMusic]",
          "[mainAudio][bgMusic]amix=inputs=2:duration=first:dropout_transition=2[aout]",
        );
      ...
      if (tempSubtitlePath) {
        filters.push(
          `subtitles=${tempSubtitlePath}:force_style='FontSize=24,PrimaryColour=&Hffffff&,OutlineColour=&H000000&,BorderStyle=3'`,
        );
      }
      ...
      command
        .on("end", async () => {
          ...
          if (tempSubtitlePath)
            await fs
              .unlink(tempSubtitlePath)
              .catch((err) =>
                this.logger.warn(
                  `Failed to delete ${tempSubtitlePath}: ${err.message}`,
                ),
              );
          if (tempThumbnailPath)
            await fs
              ...
              .catch((err) =>
                this.logger.warn(
                  `Failed to delete ${tempThumbnailPath}: ${err.message})`,
                ),
              );
  ...
  private async writeGridFSToFile(
    bucket: GridFSBucket,
    fileId: string,
    outputPath: string,
  ): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      downloadStream
        ...
        .on("error", (err) => {
          this.logger.error(
            `Failed to write GridFS file ${fileId}: ${err.message}`,
          );
          reject(err);
        });
    });
