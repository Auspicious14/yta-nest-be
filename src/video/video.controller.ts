...
import { JobModel } from 'src/schemas';
      ...
      try {
        ...
        const [tags, imageSearchQuery, description, title, videoSearchQuery] =
          await Promise.all([
            ...
            this.scriptService.generateImageSearchQuery(
              script.substring(0, 100),
            ),
            this.scriptService.generateVideoDescription(
              script.substring(0, 100),
            ),
            this.scriptService.generateVideoTitle(script.substring(0, 100)),
            this.scriptService.generateVideoSearchQuery(
              script.substring(0, 100),
            ),
          ]);
        ...
        const backgroundMusic =
          await this.pixabayService.searchAndDownloadMusic(tags[0]);
        ...
        const downloadedVideoPaths =
          await this.pixabayService.searchAndDownloadVideos(
            videoSearchQuery,
            1,
          );
